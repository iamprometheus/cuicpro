    ...
    foreach ($divisions as $index => &$division) {
      ...
      while ($number_of_teams > 0) {
        ...
        match (true) {
          ...
          $number_of_teams > 8 && $number_of_teams < 24 => $teams_this_bracket = $this->selectBracketTeams($number_of_teams, true),
          $number_of_teams >= 24 => $teams_this_bracket = $this->selectBracketTeams($number_of_teams, false),
    ...
    for ($day = 0; $day < count($this->scheduleHours); $day++) {
      ...
      for ($i = 0; $i < count($brackets); $i++) {
        ...
        for ($j = 0; $j < count($division['matches_single_elimination']); $j++) {
          ...
          $last_assigned_hour = null;
          $current_day = $day;
          $total_rounds = count($bracket['matches_per_round']);
          ...
          foreach ($bracket['matches_playoffs'] as $round => &$matches) {
            ...
            foreach ($matches as $match) {
              ...
              $is_first_match = $match[0] != "TBD" || $match[1] != "TBD";
