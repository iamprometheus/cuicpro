...
Class CoachesUserDatabase {
    ...
    public static function get_coach_by_id(int $user_id) {
        global $wpdb;
        $coach = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}cuicpro_coaches_user WHERE coach_id = $user_id" );
        return $coach;
    }
    ...
    public static function get_coach_by_name( int | null $user_id, string $coach_name) {
        global $wpdb;
        if ($user_id) {
            $coach = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}cuicpro_coaches_user WHERE coach_id != $user_id AND coach_name = '$coach_name'" );
        } else {
            $coach = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}cuicpro_coaches_user WHERE coach_name = '$coach_name'" );
        }
        return $coach;
    }
