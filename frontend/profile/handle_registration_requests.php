		...
		$html .= "<img src='{$img_official}'/>
					<form id='official-registration-form'>
						...
						<div class='form-contact-group-field'>
							...
							<label for='official-contact'>Contacto (Número de teléfono)</label>
							<input name='official-contact' type='number' placeholder='+52 123 456 7890' required/>
						</div>
...
function handle_official_account_selected()
{
	if (!isset($_POST['official-name']) || !isset($_POST['official-contact'])) {
		wp_send_json_error(array('message' => 'Faltan datos'));
		return;
	}
	$user_name = sanitize_text_field($_POST['official-name']);
	$user_contact = sanitize_text_field($_POST['official-contact']);
	$user = wp_get_current_user();
...
add_action('wp_ajax_handle_official_account_selected', 'handle_official_account_selected');
add_action('wp_ajax_nopriv_handle_official_account_selected', 'handle_official_account_selected');
